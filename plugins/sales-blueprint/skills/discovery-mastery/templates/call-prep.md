# Template: Pre-Call Prep

Template para preparar una discovery call. Completar ANTES de la llamada.

## Informacion del Prospect

```
Empresa: ___
Contacto principal: ___
Titulo/Rol: ___
LinkedIn: ___
Fuente del lead: ___
Fecha de la call: ___
Duracion agendada: ___ min
```

## Research Previo

### Empresa
- Website: ___
- Industria: ___
- Tamano (empleados): ___
- Revenue estimado: ___
- Fundada: ___
- Ubicaciones: ___
- Noticias recientes: ___

### Contacto
- Titulo actual: ___
- Tiempo en el rol: ___
- Trayectoria relevante: ___
- Publicaciones/posts recientes: ___
- Conexiones en comun: ___

### Tecnologia / Stack
- CRM: ___
- Herramientas relevantes: ___
- Tech gaps identificados: ___

## Hipotesis PULSO Pre-Call

Basado en el research, cual es tu hipotesis para cada dimension:

| Dimension | Hipotesis | Confianza (1-5) |
|-----------|----------|-----------------|
| **P - Panorama** | ___ | __ |
| **U - Urgencia** | ___ | __ |
| **L - Logro** | ___ | __ |
| **S - Situación Crítica** | ___ | __ |
| **O - Organización** | ___ | __ |

## Agenda Propuesta

```
[0:00-0:05] Apertura ACE + presentaciones
[0:05-0:10] Contexto — Panorama questions
[0:10-0:20] Diagnostico — Urgencia + Logro
[0:20-0:25] Urgencia — Situación Crítica + Organización
[0:25-0:30] Cierre — Resumen + proximos pasos
```

## Top 5 Preguntas Priorizadas

Seleccionar las 5 preguntas mas relevantes basadas en research:

1. (Panorama) ___
2. (Urgencia) ___
3. (Urgencia/Logro) ___
4. (Situación Crítica) ___
5. (Organización) ___

## Apertura ACE Personalizada

```
A: "Agradezco que hayas tomado el tiempo para esta llamada, [nombre]."
C: "Tenemos [X] minutos agendados. Sigue funcionando?"
E: "El objetivo es entender [hipotesis de dolor] y ver si tiene sentido
    explorar como podemos ayudar. Si hay fit, definimos proximos pasos."
```

## Posibles Objeciones

| Objecion anticipada | Respuesta preparada |
|---------------------|-------------------|
| "Ya tenemos una solucion" | "Entiendo. Que tan bien les esta funcionando en [area especifica]?" |
| "No tenemos presupuesto" | "Tiene sentido. Cuando planifican presupuesto para este tipo de iniciativas?" |
| "Necesito hablar con mi equipo" | "Claro. Quienes mas estarian involucrados? Podemos incluirlos en la proxima?" |
| "Envienme informacion" | "Con gusto. Para enviar algo relevante, puedo hacer una pregunta?" |

## Checklist Pre-Call

- [ ] Research de empresa completado
- [ ] Research de contacto completado
- [ ] Hipotesis PULSO documentada
- [ ] Top 5 preguntas seleccionadas
- [ ] Apertura ACE personalizada
- [ ] Tecnologia verificada (link de video, audio)
- [ ] Tabs preparadas (LinkedIn, website, CRM del prospect)
- [ ] Email de cortesia enviado (confirmacion)

## Post-Call (completar en <60 min)

### Scorecard PULSO Real

| Dimension | Score (1-5) | Evidencia |
|-----------|-------------|-----------|
| P - Panorama | __ | ___ |
| U - Urgencia | __ | ___ |
| L - Logro | __ | ___ |
| S - Situación Crítica | __ | ___ |
| O - Organización | __ | ___ |
| **TOTAL** | **__/30** | |

### Clasificacion: ___
- [ ] No calificado (5-14)
- [ ] Nurture (15-22)
- [ ] Oportunidad activa (23-30)

### Proximos Pasos Acordados
1. ___
2. ___
3. ___

### Follow-up Email Enviado: [ ] Si [ ] No
