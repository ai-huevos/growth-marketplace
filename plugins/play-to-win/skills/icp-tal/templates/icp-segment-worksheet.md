# Template: ICP Segment Worksheet

Template para documentar el Ideal Customer Profile usando diagnostico PULSO, analisis firmografico, y segmentacion basada en datos. Guia paso a paso desde la recopilacion de datos hasta la documentacion final de segmentos priorizados.

**Output Contract**: ICP documentado con segmentos priorizados, matriz firmografica completa, y perfiles de 3-5 segmentos top con PULSO tiers asignados.

**Benchmark**: Diagnostico PULSO completado para el cliente ideal, analisis cuantitativo con datos de al menos un trimestre, analisis cualitativo con 5+ entrevistas GTM, 3+ segmentos documentados con conversion rates y metricas de revenue.

---

## Seccion 1: Diagnostico PULSO del Cliente Ideal

Completar el diagnostico PULSO pensando en el cliente que *mas rapido cierra, mas facil renueva, y mas valor genera*.

### 1.1 Panorama — Contexto del Cliente Ideal

| Dimension | Descripcion del Cliente Ideal |
|---|---|
| Industria principal | ___ |
| Sub-industria o vertical | ___ |
| Tamano (empleados) | ___ |
| Revenue anual estimado | ___ |
| Etapa de crecimiento | ___ (seed / growth / scale / enterprise) |
| Stack tecnologico relevante | ___ |
| Estructura de equipo comprador | ___ |
| Geografia | ___ |

### 1.2 Urgencia — Dolor que Resolvemos Mejor que Nadie

| # | Declaración de Urgencia | Frecuencia (1-5) | Nivel (1-5) |
|---|---|---|---|
| 1 | ___ | ___ | ___ |
| 2 | ___ | ___ | ___ |
| 3 | ___ | ___ | ___ |
| 4 | ___ | ___ | ___ |
| 5 | ___ | ___ | ___ |

**Urgencia dominante** (la que mas se repite en los mejores clientes): ___

### 1.3 Logro — Impacto Cuantificable

| Tipo de Logro | Cuantificacion | Fuente del Dato |
|---|---|---|
| Revenue perdido sin solucion | $___ / año | ___ |
| Tiempo desperdiciado | ___ horas / mes | ___ |
| Oportunidades no capturadas | $___ / trimestre | ___ |
| Costo de alternativa actual | $___ / año | ___ |
| Otro: ___ | ___ | ___ |

**Logro total estimado**: $___ por año por cliente ideal.

### 1.4 Situación Crítica — Detonadores de Compra

| # | Situación Crítica | Timeline Tipico | Frecuencia |
|---|---|---|---|
| 1 | ___ | ___ meses | ___ (raro / comun / ciclico) |
| 2 | ___ | ___ meses | ___ |
| 3 | ___ | ___ meses | ___ |
| 4 | ___ | ___ meses | ___ |

**SC mas frecuente**: ___
**SC con mayor urgencia**: ___

### 1.5 Organización — Proceso de Decision

| Elemento | Descripcion |
|---|---|
| Quien inicia la busqueda | ___ (rol tipico) |
| Quien evalua opciones | ___ |
| Quien aprueba presupuesto | ___ |
| Quien puede bloquear | ___ (procurement, legal, IT) |
| Timeline tipico de decision | ___ semanas/meses |
| Criterios de evaluacion principales | ___ |
| Budget tipico disponible | $___ |
| Proceso formal (RFP, comite, etc.) | ___ |

---

## Seccion 2: Analisis Cuantitativo

### 2.1 Datos Firmograficos de Clientes Actuales

Extraer de CRM los datos de los top 20-30 clientes por LTV:

| # | Empresa | Industria | Empleados | Revenue | Ubicacion | ACV | LTV | Churn | Expansion |
|---|---|---|---|---|---|---|---|---|---|
| 1 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 2 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 3 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 4 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 5 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 6 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 7 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 8 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 9 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |
| 10 | ___ | ___ | ___ | $___ | ___ | $___ | $___ | ___% | ___% |

*(Agregar filas segun sea necesario. Recomendado: 20-30 clientes.)*

### 2.2 Cross-Reference con Lifetime Revenue

Agrupar clientes por segmento emergente y calcular metricas:

| Segmento (preliminar) | # Clientes | ACV Promedio | LTV Promedio | Churn Rate | Expansion Rate | Win Rate | Ciclo Promedio |
|---|---|---|---|---|---|---|---|
| ___ | ___ | $___ | $___ | ___% | ___% | ___% | ___ dias |
| ___ | ___ | $___ | $___ | ___% | ___% | ___% | ___ dias |
| ___ | ___ | $___ | $___ | ___% | ___% | ___% | ___ dias |
| ___ | ___ | $___ | $___ | ___% | ___% | ___% | ___ dias |

### 2.3 Fuentes de Datos Utilizadas

| Fuente | Tipo de Dato | Cobertura | Confiabilidad |
|---|---|---|---|
| CRM (___ ) | ___ | ___% de cuentas | Alta / Media / Baja |
| ___ (enrichment) | ___ | ___% de cuentas | Alta / Media / Baja |
| ___ (intent) | ___ | ___% de cuentas | Alta / Media / Baja |
| ___ (social) | ___ | ___% de cuentas | Alta / Media / Baja |

---

## Seccion 3: Analisis Cualitativo

### 3.1 Entrevistas con Equipo GTM

| # | Persona Entrevistada | Rol | Fecha | Insights Clave |
|---|---|---|---|---|
| 1 | ___ | ___ | ___ | ___ |
| 2 | ___ | ___ | ___ | ___ |
| 3 | ___ | ___ | ___ | ___ |
| 4 | ___ | ___ | ___ | ___ |
| 5 | ___ | ___ | ___ | ___ |

#### Preguntas y Respuestas Consolidadas

**P1: "Describe al cliente que mas rapido cierra y mas facil renueva"**
Respuesta consolidada: ___

**P2: "Cual es el pain que mencionan en la primera llamada?"**
Respuesta consolidada: ___

**P3: "Que critical event dispara la urgencia de comprar?"**
Respuesta consolidada: ___

**P4: "Donde pierdes mas deals y por que?"**
Respuesta consolidada: ___

**P5: "Si pudieras clonar 10 cuentas, cuales serian y que tienen en comun?"**
Respuesta consolidada: ___

#### Patrones Identificados en Entrevistas

| Patron | Veces Mencionado | Por Quien | Implicacion para ICP |
|---|---|---|---|
| ___ | ___/5 | ___ | ___ |
| ___ | ___/5 | ___ | ___ |
| ___ | ___/5 | ___ | ___ |

### 3.2 Deep Dive PULSO: Clientes Top

Completar una tarjeta PULSO por cada cliente top seleccionado:

#### Cliente 1: _______________

| Campo | Detalle |
|---|---|
| **Panorama** | ___ |
| **Urgencia** | ___ |
| **Logro** (cuantificado) | $___ |
| **Situación Crítica** | ___ |
| **Organización** (proceso) | ___ |
| **ACV** | $___ |
| **Time to Close** | ___ dias |
| **Expansion** | ___% |
| **NPS/CSAT** | ___ |
| **Razon principal de compra** | ___ |

#### Cliente 2: _______________

| Campo | Detalle |
|---|---|
| **Panorama** | ___ |
| **Urgencia** | ___ |
| **Logro** (cuantificado) | $___ |
| **Situación Crítica** | ___ |
| **Organización** (proceso) | ___ |
| **ACV** | $___ |
| **Time to Close** | ___ dias |
| **Expansion** | ___% |
| **NPS/CSAT** | ___ |
| **Razon principal de compra** | ___ |

#### Cliente 3: _______________

| Campo | Detalle |
|---|---|
| **Panorama** | ___ |
| **Urgencia** | ___ |
| **Logro** (cuantificado) | $___ |
| **Situación Crítica** | ___ |
| **Organización** (proceso) | ___ |
| **ACV** | $___ |
| **Time to Close** | ___ dias |
| **Expansion** | ___% |
| **NPS/CSAT** | ___ |
| **Razon principal de compra** | ___ |

*(Duplicar para clientes 4-10.)*

---

## Seccion 4: Segmentos Identificados

Con los datos cuantitativos y cualitativos consolidados, documentar los segmentos que emergen:

| Atributo | Segmento 1: ___ | Segmento 2: ___ | Segmento 3: ___ | Segmento 4: ___ | Segmento 5: ___ |
|---|---|---|---|---|---|
| **Industria** | ___ | ___ | ___ | ___ | ___ |
| **Sub-vertical** | ___ | ___ | ___ | ___ | ___ |
| **Tamano (empleados)** | ___ | ___ | ___ | ___ | ___ |
| **Revenue** | $___ | $___ | $___ | $___ | $___ |
| **Ubicacion** | ___ | ___ | ___ | ___ | ___ |
| **Pain dominante** | ___ | ___ | ___ | ___ | ___ |
| **Critical Event** | ___ | ___ | ___ | ___ | ___ |
| **ACV promedio** | $___ | $___ | $___ | $___ | $___ |
| **Win Rate** | ___% | ___% | ___% | ___% | ___% |
| **Ciclo (dias)** | ___ | ___ | ___ | ___ | ___ |
| **LTV** | $___ | $___ | $___ | $___ | $___ |
| **Churn Rate** | ___% | ___% | ___% | ___% | ___% |
| **Expansion Rate** | ___% | ___% | ___% | ___% | ___% |

---

## Seccion 5: Perfil de Segmento Detallado

*(Duplicar esta seccion por cada segmento priorizado.)*

### Segmento: _______________

**Descripcion narrativa** (2-3 oraciones):
___

#### Atributos Firmograficos

| Atributo | Valor Ideal | Rango Aceptable | Descalificante |
|---|---|---|---|
| Industria | ___ | ___ | ___ |
| Empleados | ___ | ___ | ___ |
| Revenue | ___ | ___ | ___ |
| Geografia | ___ | ___ | ___ |
| Tecnologia | ___ | ___ | ___ |
| Crecimiento YoY | ___ | ___ | ___ |

#### PULSO del Segmento

| Componente | Detalle Especifico del Segmento |
|---|---|
| **Panorama** | ___ |
| **Urgencia** | ___ |
| **Logro** | $___ |
| **Situación Crítica** | ___ |
| **Organización** | ___ |

#### Metricas Historicas

| Metrica | Valor | Fuente |
|---|---|---|
| ACV promedio | $___ | CRM |
| Win Rate | ___% | CRM |
| Ciclo de venta | ___ dias | CRM |
| LTV | $___ | CRM |
| Churn anual | ___% | CS |
| Expansion anual | ___% | CS |
| NPS promedio | ___ | CS |

#### Estrategia de Engagement Recomendada

- **Messaging principal**: ___
- **Play recomendado** (ver `frameworks/four-plays-relationship.md`): ___
- **Canal primario**: ___
- **Contenido clave**: ___
- **Objeciones tipicas**: ___

---

## Seccion 6: PULSO Tiers — Asignacion Final

Asignar cada segmento a un PULSO Tier:

| Segmento | P Match | U Urgencia | L Cuantificado | SC Activa | Tier Asignado | Justificacion |
|---|---|---|---|---|---|---|
| ___ | Alto/Medio/Bajo | Alto/Medio/Bajo | Si/No | Si/No | T1/T2/T3 | ___ |
| ___ | Alto/Medio/Bajo | Alto/Medio/Bajo | Si/No | Si/No | T1/T2/T3 | ___ |
| ___ | Alto/Medio/Bajo | Alto/Medio/Bajo | Si/No | Si/No | T1/T2/T3 | ___ |
| ___ | Alto/Medio/Bajo | Alto/Medio/Bajo | Si/No | Si/No | T1/T2/T3 | ___ |
| ___ | Alto/Medio/Bajo | Alto/Medio/Bajo | Si/No | Si/No | T1/T2/T3 | ___ |

### Reglas de Asignacion

- **T1**: Panorama fuerte + Urgencia critica + Logro cuantificado + SC activa → Todas las condiciones
- **T2**: Panorama bueno + Urgencia clara + Logro medible → Sin SC requerida
- **T3**: Panorama parcial + Urgencia leve → Minimo viable

### Anti-ICP: Clientes a Evitar

| Atributo | Descripcion | Por Que Evitar |
|---|---|---|
| ___ | ___ | ___ |
| ___ | ___ | ___ |
| ___ | ___ | ___ |

---

## Seccion 7: Plan de Accion Post-ICP

| Accion | Responsable | Deadline | Estado |
|---|---|---|---|
| Presentar ICP al equipo GTM completo | ___ | ___ | Pendiente |
| Actualizar criterios de scoring en CRM | ___ | ___ | Pendiente |
| Alinear messaging de Marketing por segmento | ___ | ___ | Pendiente |
| Crear TAL basado en ICP (ver `templates/tal-tiering-calculator.md`) | ___ | ___ | Pendiente |
| Programar review trimestral de ICP | ___ | ___ | Pendiente |

---

## Metadata

| Campo | Valor |
|---|---|
| **Template ID** | T-49 |
| **Skill** | `play-to-win/icp-tal` |
| **Framework** | `frameworks/icp-pulso-methodology.md` |
| **Version** | 1.0.0 |
| **Tiempo estimado** | 2-4 semanas (recopilacion de datos + analisis + documentacion) |
| **Prerequisitos** | Acceso a CRM con datos de al menos 1 trimestre, disponibilidad de equipo GTM para entrevistas |
| **Output** | ICP Summary + 3-5 Account Segment Profiles + PULSO Tiers + Anti-ICP + Plan de Accion |
| **Siguiente paso** | `templates/tal-tiering-calculator.md` para construir el TAL basado en este ICP |
