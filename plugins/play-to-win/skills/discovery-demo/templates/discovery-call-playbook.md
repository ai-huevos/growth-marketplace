# Template: Discovery Call Playbook (T-53)

Template para preparar y ejecutar discovery calls usando ACE opening y SPICED diagnostic progression. Produce un playbook completo listo para ejecutar, con scripts personalizados, preguntas con contexto, y plan de follow-up.

**Output Contract**: Playbook completo de discovery call con scripts ACE personalizados, checklist de 54 puntos, agenda alineada a dolores investigados, preguntas SPICED con contexto, historias de terceros preparadas, y setup de next meeting.

**Benchmark**: ACE opening scripted y personalizado, 5+ preguntas SPICED preparadas con contexto de research, agenda enviada pre-call, checklist completado por fase, next meeting secured con fecha y stakeholders adicionales.

---

## Seccion 1: Preparacion Pre-Call

### Datos del Prospect

| Campo | Informacion |
|-------|-------------|
| **Empresa** | ___ |
| **Industria** | ___ |
| **Tamano** (empleados/revenue) | ___ |
| **Persona de contacto** | ___ |
| **Rol/titulo** | ___ |
| **LinkedIn URL** | ___ |
| **Origen del lead** | Inbound / Outbound / Referido por ___ |
| **Fecha de la llamada** | ___ |
| **Duracion agendada** | ___ minutos |

### Investigacion LinkedIn

| Dato | Hallazgo | Como usarlo en la conversacion |
|------|----------|-------------------------------|
| **Background profesional** | ___ | ___ |
| **Tiempo en el puesto actual** | ___ | ___ |
| **Publicaciones/articulos recientes** | ___ | ___ |
| **Conexiones en comun** | ___ | ___ |
| **Tamano del equipo** | ___ | ___ |

### Notas de conversaciones previas

| Fecha | Canal | Temas cubiertos | Datos SPICED obtenidos |
|-------|-------|-----------------|----------------------|
| ___ | ___ | ___ | ___ |
| ___ | ___ | ___ | ___ |

### Insights notables (3 minimo)

1. **Insight**: ___ | **Fuente**: ___ | **Pregunta que genera**: ___
2. **Insight**: ___ | **Fuente**: ___ | **Pregunta que genera**: ___
3. **Insight**: ___ | **Fuente**: ___ | **Pregunta que genera**: ___

---

## Seccion 2: ACE Opening Script

### A — Appreciate (10 seg)

Seleccionar la variacion apropiada y personalizar:

- [ ] **Referido**: "Gracias por aceptar la recomendacion de ___. Me conto que ___."
- [ ] **Inbound**: "Gracias por contactarnos. Vi que descargaste ___. Eso me dice que ___ es prioridad para ti."
- [ ] **Outreach**: "Gracias por darme estos minutos, ___. Se que tu agenda esta apretada y quiero que cada segundo cuente."
- [ ] **Personalizado**: ___

**Script final**:
> "___"

### C — Check End Time (10 seg)

**Script**:
> "Tenemos ___ minutos agendados. Sigue siendo buen tiempo para ti, o tienes algun compromiso antes?"

**Plan B si tienen menos tiempo**: Si dicen ___ minutos, priorizar: ___

### E — End Goal (25 seg)

**Script**:
> "Mi objetivo hoy es ___. Al final quiero que tu tengas claridad sobre ___."

**Chunking** (desglose del objetivo):
> "Para lograr eso, primero ___, segundo ___, y tercero ___."

### Post-ACE: Pregunta Poderosa

**Script de transicion**:
> "Tengo una agenda preparada basada en lo que investigue, pero antes de seguir con ella... que mas quisieras obtener de la reunion de hoy?"

**Plan si hay multiples participantes**:
- Participantes esperados: ___
- Si <5: preguntar a cada uno
- Si >5: preguntar al champion (___)

---

## Seccion 3: Agenda

### Agenda propuesta

| # | Tema | Tiempo estimado | Conecta con |
|---|------|----------------|-------------|
| 1 | ACE + contexto | 3 min | Establecer confianza |
| 2 | ___ | ___ min | Dolor hipotesis: ___ |
| 3 | ___ | ___ min | Dolor hipotesis: ___ |
| 4 | ___ | ___ min | Dolor hipotesis: ___ |
| 5 | Resumen + next steps | 5 min | Wagons + follow-up |
| | **Total** | **___ min** | |

### Email de agenda pre-call

```
Asunto: Agenda para nuestra reunion del [fecha] — [empresa]

Hola [nombre],

Gracias por agendar esta conversacion. Para maximizar nuestro tiempo, aqui la agenda propuesta:

1. Contexto e introductions (3 min)
2. [Tema 1] (X min)
3. [Tema 2] (X min)
4. [Tema 3] (X min)
5. Resumen y siguientes pasos (5 min)

Si hay algo que quieras agregar o cambiar, me avisas.

Nos vemos el [fecha] a las [hora].

[firma]
```

---

## Seccion 4: Preguntas SPICED con Contexto

### Situation (2-4 preguntas cerradas con contexto)

| # | Pregunta | Contexto que la respalda | Respuesta esperada |
|---|----------|-------------------------|-------------------|
| S1 | "___ ... es correcto?" | Fuente: ___ | ___ |
| S2 | "___ ... sigue siendo asi?" | Fuente: ___ | ___ |
| S3 | "___ ... es tu equipo principal?" | Fuente: ___ | ___ |
| S4 | "___ ... confirmas?" | Fuente: ___ | ___ |

**Guia**: Cada pregunta de situacion debe demostrar que investigaste. Usar datos de LinkedIn, website, reportes publicos, o conversaciones previas.

### Pain (1-2 preguntas reflexivas con contexto)

| # | Pregunta | Contexto que la respalda | Dolor que explora |
|---|----------|-------------------------|-------------------|
| P1 | "Con [contexto], como manejan ___?" | ___ | ___ |
| P2 | "Muchos equipos similares nos cuentan que ___. Les pasa algo parecido?" | ___ | ___ |

**Guia**: Nunca usar "que te quita el sueno?" ni "cuales son tus problemas?". Las preguntas de dolor son reflexivas — invitan a pensar, no a confesar.

### Impact (cuantificacion)

| # | Pregunta | Metrica que busca | Dimension |
|---|----------|-------------------|-----------|
| I1 | "Si resolvieras ___, cual seria el impacto en ___?" | ___ | Revenue / Costo / CX / Eficiencia |
| I2 | "Cuanto estiman que pierden/gastan por ___?" | ___ | Revenue / Costo / CX / Eficiencia |

### Critical Event

| # | Pregunta | Timeline que explora |
|---|----------|---------------------|
| CE1 | "Hay alguna fecha o evento que haga esto urgente?" | ___ |
| CE2 | "Que pasa si no se resuelve para [fecha]?" | ___ |

### Decision

| # | Pregunta | Informacion que busca |
|---|----------|----------------------|
| D1 | "Quien mas necesita estar involucrado?" | Stakeholders |
| D2 | "Como han tomado decisiones similares antes?" | Proceso |
| D3 | "Que criterios son los mas importantes para ustedes?" | Criterios |

---

## Seccion 5: Historias de Terceros Preparadas

### Historia 1

| Campo | Contenido |
|-------|-----------|
| **Cliente/Industria** | ___ |
| **Situacion similar** | ___ |
| **Dolor identico** | ___ |
| **Lo que hicieron** | ___ |
| **Resultado cuantificable** | ___ |
| **Frase de entrada** | "Esto lo escucho mucho entre equipos de ___..." |
| **Usar cuando el prospect diga** | ___ |

### Historia 2

| Campo | Contenido |
|-------|-----------|
| **Cliente/Industria** | ___ |
| **Situacion similar** | ___ |
| **Dolor identico** | ___ |
| **Lo que hicieron** | ___ |
| **Resultado cuantificable** | ___ |
| **Frase de entrada** | "No estas solo en esto. Un cliente nuestro en ___..." |
| **Usar cuando el prospect diga** | ___ |

### Historia 3 (backup)

| Campo | Contenido |
|-------|-----------|
| **Cliente/Industria** | ___ |
| **Situacion similar** | ___ |
| **Dolor identico** | ___ |
| **Lo que hicieron** | ___ |
| **Resultado cuantificable** | ___ |
| **Frase de entrada** | "Justo la semana pasada hable con alguien que ___..." |
| **Usar cuando el prospect diga** | ___ |

---

## Seccion 6: Checklist de 54 Puntos

### Antes de la llamada

- [ ] Research LinkedIn completado (persona + empresa)
- [ ] Notas de conversaciones previas revisadas
- [ ] 3+ insights notables identificados para usar como contexto

### Abrir la reunion

- [ ] Llegar 2 minutos antes a la videollamada
- [ ] Camara encendida, fondo profesional, audio verificado
- [ ] Notas/preguntas visibles (no en pantalla compartida)
- [ ] Energia alta pero natural

### Ejecutar ACE

- [ ] **A** — Appreciate personalizado ejecutado (10 seg)
- [ ] **C** — Check End Time ejecutado (10 seg)
- [ ] **E** — End Goal con chunking ejecutado (25 seg)
- [ ] Agenda presentada
- [ ] Pregunta poderosa realizada a todos los participantes

### Diagnosticar con SPICED

- [ ] **S** — 2+ preguntas de Situation cerradas con contexto
- [ ] **P** — 1+ pregunta de Pain reflexiva con contexto
- [ ] **Summarize** — "Entonces tienes [S] que causa [P]... capte bien?"
- [ ] **Story** — Historia de terceros compartida post-resumen
- [ ] **I** — Impact cuantificado en 2+ dimensiones
- [ ] **CE** — Critical Event identificado con timeline
- [ ] **D** — Criterios, proceso y stakeholders mapeados

### Cerrar

- [ ] Resumen de 3 hallazgos principales
- [ ] Next steps concretos propuestos
- [ ] Fecha/hora de siguiente reunion definida
- [ ] Stakeholders adicionales identificados para invitar
- [ ] Responsable del resumen definido (tu o el prospect)
- [ ] Agradecimiento final genuino

### Post-call

- [ ] Email follow-up enviado (dentro de 2 horas)
- [ ] SPICED findings documentados en CRM
- [ ] Relationship map actualizado
- [ ] Agenda siguiente reunion preparada
- [ ] Recursos mencionados compartidos

### Control de calidad

- [ ] El prospect hablo 60%+ del tiempo
- [ ] 2+ dolores validados identificados
- [ ] 1+ impacto concreto cuantificado
- [ ] Critical Event real establecido
- [ ] Siguiente reunion con fecha, hora y agenda

### Preparacion de informacion

- [ ] SPICED summary redactado para transferir a demo
- [ ] Historias de terceros relevantes seleccionadas
- [ ] Demo points preliminares alineados a dolores
- [ ] Decision criteria mapeados
- [ ] Stakeholder map actualizado

### Gestion de objeciones

- [ ] Objeciones de timing manejadas con Critical Event
- [ ] Objeciones de presupuesto reenmarcadas como costo de no actuar
- [ ] Objeciones de "ya tenemos solucion" exploradas en profundidad
- [ ] Objeciones de "necesito consultarlo" convertidas en invitacion
- [ ] Objeciones de alcance delimitadas y priorizadas

### Documentacion final

- [ ] Notas en formato SPICED
- [ ] Action items con responsable y fecha
- [ ] Follow-up email enviado con resumen
- [ ] CRM actualizado
- [ ] Calendario actualizado con siguiente reunion

### Reflexion

- [ ] Autoevaluar ACE opening (1-5): ___
- [ ] 1 cosa para mejorar: ___

---

## Seccion 7: Connecting the Wagons

### Setup de siguiente reunion

| Campo | Contenido |
|-------|-----------|
| **Fecha/hora propuesta** | ___ |
| **Duracion** | ___ minutos |
| **Objetivo** | ___ |
| **Formato** | Demo / Diagnostico profundo / Reunion con sponsor |
| **Tu preparacion necesaria** | ___ |

### Expansion de stakeholders

| Stakeholder a invitar | Rol | Razon para incluirlo | Quien lo invita |
|-----------------------|-----|---------------------|-----------------|
| ___ | ___ | ___ | ___ |
| ___ | ___ | ___ | ___ |

### Script para connecting wagons

> "Basado en lo que conversamos hoy, creo que el siguiente paso natural seria ___. Sugiero reunirnos el [fecha] para [objetivo]. Para maximizar el valor, seria genial tener a [persona/rol]. Puedes facilitarnos esa conexion?"

---

## Seccion 8: Post-Call Actions

### Template de email follow-up

```
Asunto: Resumen de nuestra conversacion — [empresa] + [tu empresa]

Hola [nombre],

Gracias por la conversacion de hoy. Aqui un resumen de lo que cubrimos:

**Lo que aprendimos:**
- [Situacion 1]
- [Situacion 2]

**Desafios identificados:**
- [Pain 1]
- [Pain 2]

**Impacto potencial:**
- [Impact cuantificado]

**Siguientes pasos:**
- [Accion 1] — Responsable: ___ — Fecha: ___
- [Accion 2] — Responsable: ___ — Fecha: ___

**Proxima reunion:** [fecha] a las [hora] — Objetivo: [objetivo]

Si me falto algo, hazmelo saber.

[firma]
```

### Documentacion SPICED para CRM

| Campo SPICED | Contenido del discovery |
|-------------|------------------------|
| **Situation** | ___ |
| **Pain** | ___ |
| **Impact** | ___ |
| **Critical Event** | ___ |
| **Decision** | Criterios: ___ / Proceso: ___ / Stakeholders: ___ |

---

## Metadata

| Campo | Valor |
|-------|-------|
| **Template ID** | T-53 |
| **Skill** | discovery-demo |
| **Plugin** | play-to-win |
| **Version** | 1.0.0 |
| **Frameworks usados** | `frameworks/ace-discovery-call.md`, `frameworks/diagnosis-deep-dive.md` |
| **Basado en** | Blueprint #25 (Perfect Discovery Call), Blueprint #26 (How to Diagnose) |
| **Output** | Discovery Call Playbook completo |
| **Tiempo estimado de preparacion** | 30-45 minutos |
| **Nivel de complejidad** | Medio |
