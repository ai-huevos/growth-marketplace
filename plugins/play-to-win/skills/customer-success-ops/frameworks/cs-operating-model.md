...
Framework completo del Customer Success Operating Model de Winning by Design. Este documento es la referencia condensada de todas las fases, sub-pasos, metricas y habilidades core del modelo.
...
|-----------|------------|---------|
| **S — Situation** | Hechos y circunstancias relevantes | Tamano de empresa, ubicacion, empleados, software, necesidades de seguridad, nivel de madurez, metas de revenue |
| **P — Pain** | Problema u oportunidad que necesita solucion | A menudo expresado superficialmente — requiere diagnostico mas profundo |
| **I — Impact** | Resultados de resolver el dolor | Emocional (beneficia al individuo primero) + Racional (beneficia a la empresa primero) |
| **CE — Critical Event** | Deadline con consecuencias negativas si se pierde | Impulsa comportamiento: completar implementacion, expandir uso, renovar |
| **D — Decision** | Personas involucradas, proceso, criterios | Quien firma, quien influye, que criterios usan para evaluar |

**Propiedades clave de SPICED**:
- Es **secuencial**: necesitas entender la Situacion antes de comprender Pain e Impact
- Refleja **relevancia contextual**: contactar basado en situacion tiene menos relevancia que basado en impacto
- Una **Decision** resulta de la necesidad de Impacto del cliente — no la impulsa
...
**Secuencia de preguntas para descubrir impacto**:
1. Situation (cerrada): "Cuantos usuarios hay en la herramienta actual?"
2. Pain (abierta): "Con que siguen luchando los usuarios? Como se manifiesta?"
3. Impact (abierta): "Cuanto tiempo o dinero se podria ahorrar al hacer mas rapido X?"
...
- **Critical Event**: Tiene fecha Y consecuencia negativa. "Tu renovacion es el 15 de septiembre, y si no se procesa, tu equipo volvera a gestionar gastos en spreadsheets."

Los Critical Events causan priorizacion, impulsando decisiones en negocios de revenue recurrente.
...
2. Transferir info usando SPICED (estructura org, personas, red flags)
3. Guias claras: Quien compro? Por que? Que hacian antes? Por que no funcionaba? Que problema resolver?

**Flujo**: PREPARE → SETUP (ACE) → SPICED verify → STRATEGIZE → CLOSE (WAGONS) → FOLLOW-UP
...
**Flujo**: PREPARE (internal + external) → SETUP (ACE) → SPICED verify/deepen → TEACHING MOMENTS → ONBOARDING REVIEW → JIP alignment → NEXT STEPS → WAGONS → POST CALL ACTION
...
**En cada revision usar SPICED para validar**:
...
El JIP orquesta todas las acciones. Entrenar a todos los roles customer-facing a usar SPICED para impulsar impacto.
...
**Flujo**: PREPARE (stories) → SETUP (ACE) → SPICED validate → STORY (cliente que cambio estrategia) → IMPACT REVIEW → STORY + DEMO → CLOSE (nuevos CE para 12 meses) → FOLLOW-UP (resumen 5 min)
...
**Para cada trigger ejecutado**: Accion probada → Medir impacto → Registrar como pocket story con SPICED.
...
**Flujo**: PREPARE → SETUP (ACE) → SPICED validate → IMPACT review → STORY → DEMO → DECISION → CE → CLOSE → FOLLOW-UP
...
**Meta**: Priorizar expansion basada en Impacto + Critical Event (no solo Situacion/Pain).

**Matriz**: Para cada producto/servicio, evaluar nivel de Impacto y presencia de Critical Event. Priorizar donde ambos son altos.

### E3. Account Planning
**Meta**: Coordinar actividades de cuenta alrededor de metas del cliente.

**3 elementos**: Impacto + Critical Event + Decision Process
...
| S2 | Diagnose Sequence | OPEN (ACE) → SITUATION → PAIN → STORY → IMPACT → CE → DECISION → CLOSE |
| S3 | Critical Event Blueprint | Diferenciar Compelling Event (fecha sin urgencia) de Critical Event (fecha + consecuencia negativa) |
| S4 | Critical Event Timeline | Identificar CE → Consecuencias → Pasos en orden cronologico inverso |
| S5 | Navigating an Organization | Initiator → User → Champion → Gatekeeper → Influencer → Decision Maker → Executive Buyer |
| S6 | Customer Story | 3 partes: Situacion similar → Pain (consecuencias negativas) → Impacto positivo de la solucion |
...
6. Codificar SPICED en cada Blueprint para que sea entrenable y mejorable
7. Medir metricas [M, CR, PM] y mejorar con el tiempo

---

*Fuente: Blueprint #06 — CS Operating Model, Winning by Design*
