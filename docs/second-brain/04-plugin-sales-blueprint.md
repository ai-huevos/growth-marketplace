...
- **Versión**: 1.0.0
- **Propósito**: Sistema de ejecución de ventas B2B basado en Winning by Design
- **Autor**: AI Huevos
- **Metodología base**: Winning by Design / SPICED
...
SPICED conecta discovery (diagnosticar) → pipeline (evaluar) → proposals (presentar) → coaching (mejorar). Es el hilo conductor de todo el sistema.
...
### Qué hace
Preparación y ejecución de discovery calls con apertura ACE, diagnóstico SPICED, y framework de 7 Momentos.
...
2. Hipótesis SPICED pre-call
3. Preguntas priorizadas por dimensión SPICED
...
- `frameworks/spiced-framework.md`: Guía completa de SPICED con scoring
...
### Qué hace
Gestión de pipeline con SPICED scoring, indicadores de salud, modelo Bowtie, y métricas core.
...
Sistema rojo/amarillo/verde basado en SPICED score por deal.
...
### Qué hace
Generación de propuestas comerciales basadas en el diagnóstico SPICED. Prerrequisito: SPICED >= 23.
...
### Rol
SDR senior que investiga prospectos, califica usando SPICED, y prepara outreach personalizado.

### Workflow (5 fases)
1. **Research del Prospect**: Empresa, contacto, stack tecnológico, compelling events
2. **Hipótesis SPICED**: Score pre-contacto por dimensión con nivel de confianza
...
- Hipótesis SPICED con confianza por dimensión
...
### Workflow (5 fases)
1. **Intake**: Básicos del deal + SPICED actual + historia + competencia
2. **Análisis SPICED Crítico**: Evalúa cada dimensión con escepticismo profesional (¿es dato de primera mano o inferido? ¿es "happy ears"?)
...
5. **Mutual Action Plan**: Si SPICED >= 20, plan con pasos, owners y fechas
...
### Output
- Deal Assessment con SPICED score crítico
...
### /discovery
Prepara una discovery call interactiva en 6 fases: Research → SPICED hypothesis → Questions → ACE agenda → Objections → Checklist.

### /propuesta
Genera una propuesta B2B completa en 6 fases: SPICED intake → Segment → Info gathering → Generate → Trading prep → Review.

### /pipeline
Review de pipeline en 7 fases: Data → Snapshot → SPICED por deal → Health scoring → Risks → Forecast → Actions.
